...
(defsubst xmltok-attribute-raw-normalized-value (att)
  "Return an object representing the normalized value of ATTLIST.
...
(eval-when-compile
  (let* ((or "\\|")
    ...
    (xmltok-defregexp
     ...
     (let* ((literal-content "[-._a-zA-Z0-9]*")
	    (literal
...
literal.  Adds to `xmltok-errors' and `xmltok-dependent-regions' as
appropriate."
  (let ((case-fold-search nil)
	xmltok-start
	xmltok-type
