...
(make-variable-buffer-local 'rng-compile-table)
...
(make-variable-buffer-local 'rng-ipattern-table)

(defvar rng-last-ipattern-index nil)
(make-variable-buffer-local 'rng-last-ipattern-index)
...
(defun rng-normalize-group-list (ipatterns)
  "Normalize a list containing members of a group, expanding nested
groups, removing empty members, handling notAllowed.  Return a pair
whose car says whether the list is nullable and whose cdr is the
normalized list."
  (let ((nullable t)
	...
	(cond ((eq (rng-ipattern-get-type member) 'group)
	       (let ((sub (rng-ipattern-get-child member)))
		 (while sub
		   (setq tail (setcdr tail (cons (car sub) nil)))
		   (setq sub (cdr sub))))
	       (setq cur (cdr cur))
	       (setcdr tail cur))
	      ((eq member rng-empty-ipattern)
	       (setq cur (cdr cur))
	       (setcdr tail cur))
	      ((eq member rng-not-allowed-ipattern)
	       (setcdr head (cons rng-not-allowed-ipattern nil))
	       (setq cur nil)
	       (setq nullable nil))
	      (t
	       (setq tail cur)
	       (setq cur (cdr cur))))))
    (setcar head nullable)
    head))
  ...
  (let ((sorted t)
	...
	(cond ((eq (rng-ipattern-get-type member) 'interleave)
		 ...
		 (while sub
	       ...
	       (and sorted
		    (or (and (<= prev-index
				 (rng-ipattern-get-index
				  (car (rng-ipattern-get-child member))))
			     (setq prev-index
				   (rng-ipattern-get-index (car tail))))
			(setq sorted nil)))
	       (setq cur (cdr cur))
	       (setcdr tail cur))
	      ((eq member rng-empty-ipattern)
	      ...
	      (t
	       ...
	       (setq tail cur)
	       (setq cur (cdr cur))))))
    (cond (sorted (setcar head nullable) head)
	  (t (cons nullable 
		   (sort (cdr head) 'rng-compare-ipattern))))))
...
(defun rng-name-class-possible-names (nc namespaces accum)
...
pair, where NAMESPACE is a symbol or t or nil and LOCAL-NAME is a
string or t. t for the LOCAL-NAME means any name in that namespace is
allowed. t for the NAMESPACE means any namespace is allowed \(the cdr
must be t in this case).  nil for NAMESPACE matches the absent
namespace.  ACCUM is a list of names which should be appended to the
returned list. The returned list may contain duplicates.

NAMESPACES is a list of symbols specifying known namespaces.  If any
local name from a particular known namespace X is allowed, then the
list of possible names will contain a (X . t) member.  The absent
namespace is always treated as a known namespace.  If a name with a
particular namespace is possible, it is included even if its namespace
is not known.  If a particular namespace is known to be possible
because of a namespace wildcard, then it is included even if that
namespace is not known.  Known namespaces only make a difference for
any-name-except name classes."
  (if (consp nc)
      (cons nc accum)
    (let ((type (aref nc 0)))
      (cond ((eq type 'any-name)
	     (append '((t . t) (nil . t))
		     (append (mapcar (lambda (ns) (cons ns t)) namespaces)
			     accum)))
	    ((eq type 'any-name-except)
	     (let ((except (aref nc 1)) ns)
	       (while namespaces
		 (setq ns (car namespaces))
		 (when (not (rng-name-class-contains except
						     (cons ns "")))
		   (setq accum (cons (cons ns t) accum)))
		 (setq namespaces (cdr namespaces)))
	       (when (not (rng-name-class-contains except (cons nil "")))
		 (setq accum (cons '(nil . t) accum))))
	     (cons '(t . t) accum))
	    ((memq type '(ns-name ns-name-except))
	     (cons (cons (aref nc 1) t) accum))
	    ((eq type 'choice)
...
(defun rng-search-element (name pattern contains-fun)
  (let ((searched (make-hash-table :test 'eq))
	type todo patterns found)
    (while (progn
	     (setq type (car pattern))
	     (cond ((eq type 'element)
		    (when (funcall contains-fun name (nth 1 pattern))
		      (setq found
			    (cons (rng-compile (nth 2 pattern))
				  found)))
...
(defun rng-ipattern-possible-start-tags (ipattern namespaces accum)
  (let ((type (rng-ipattern-get-type ipattern)))
    (cond ((eq type 'after)
...
(defun rng-ipattern-possible-attributes (ipattern namespaces accum)
  (let ((type (rng-ipattern-get-type ipattern)))
    (cond ((eq type 'after)
...
(defun rng-match-out-of-context-start-tag-open (name)
  (let* ((found (rng-search-element name
				    rng-current-schema
				    'rng-search-name))
...
(defun rng-match-infer-start-tag-namespace (local-name)
  (let ((ncs (rng-ipattern-possible-start-tags rng-match-state nil nil))
	(nc nil)
	(ns nil))
    (while ncs
      (setq nc (car ncs))
...
(defun rng-match-possible-start-tag-names (namespaces)
...
where NAMESPACE is a symbol or t or nil and LOCAL-NAME is a string or
t. t for the LOCAL-NAME means any name in that namespace is allowed. t
for the NAMESPACE means any namespace is allowed (the cdr must be t in
this case).  nil for NAMESPACE matches the absent namespace.  The
returned list may contain duplicates.

NAMESPACES is a list of symbols specifying known namespaces.  If all
local name from a particular known namespace X are allowed, then the
returned list of possible names will contain a (X . t) member.  The
absent namespace is always treated as a known namespace.  If a name
with a particular namespace is possible, it is included even if its
namespace is not known.  If all local names from a particular
namespace X are known to be possible because of a namespace wildcard,
then (X . t) is included even if X is not known.  Known namespaces
only make a difference for any-name-except name classes."
  (rng-ipattern-possible-start-tags rng-match-state namespaces nil))

(defun rng-match-possible-attribute-names (namespaces)
...
more information."
  (rng-ipattern-possible-attributes rng-match-state namespaces nil))
