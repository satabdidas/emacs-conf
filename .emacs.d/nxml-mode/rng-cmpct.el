...
;;;###autoload
(defun rng-c-set-schema (filename)
  "Set the schema for the current buffer to the schema in FILENAME.
FILENAME must be the name of a file containing a RELAX NG schema
in the compact \(non-XML) syntax. The association is between
the buffer and the schema: the association is lost when the buffer
is killed."
  (interactive "fRELAX NG compact schema: ")
  (condition-case err
      (progn
	(setq rng-current-schema (rng-c-parse-file filename))
...
(put 'rng-c-incorrect-schema
     'error-conditions
     '(error rng-error rng-c-incorrect-schema))
...
(defun rng-c-signal-incorrect-schema (filename pos message)
  (signal 'rng-c-incorrect-schema
	  (list filename pos message)))
...
(defun rng-c-declare-standard-namespaces ()
  (setq rng-c-namespace-decls
	(cons (cons "xml" rng-xml-namespace-uri)
...
(defun rng-c-declare-standard-datatypes ()
  (setq rng-c-datatype-decls
	(cons (cons "xsd" rng-xsd-namespace-uri)
...
(defun rng-c-parse-datatypes ()
  (let ((prefix (rng-c-parse-identifier-or-keyword)))
    ...
    (setq rng-c-datatype-decls
	  (cons (cons prefix
		      (rng-make-datatype-uri (rng-c-parse-literal)))
		rng-c-datatype-decls))))
...
(defun rng-c-declare-namespace (declare-default prefix)
  ...
  (let ((ns (cond ((string-equal rng-c-current-token "inherit")
		   rng-c-inherit-namespace)
		  (t
		   (rng-make-namespace (rng-c-parse-literal))))))
    (and prefix
...
(defun rng-c-expand-file (uri)
  ;;; XXX should do much more checking
  (expand-file-name uri
		    (file-name-directory rng-c-file-name)))
